  ...
  return (
    <div className="space-y-2">
      ...
      <div className="hidden gap-3 px-2 lg:grid lg:grid-cols-3">
        <p className="text-center text-xs italic smallcaps text-muted-foreground">
          Manual / Uploaded
        </p>
        <p className="flex items-center justify-center gap-2 text-center text-xs italic smallcaps text-muted-foreground">
          AI Recommendation
          {data.training_phase && (
            <Badge variant="outline" className={cn('text-[10px] capitalize', phaseColor(data.training_phase))}>
      ...
      {days.map((dateStr) => {
        ...
        return (
          <div
            key={dateStr}
            className={cn(
              'rounded-none border-y border-foreground/20 bg-transparent p-3',
              today && 'border-y-2 border-foreground',
            )}
          >
            {/* Day header */}
            <div className="mb-3 flex items-center justify-between">
              <div className="flex items-baseline gap-2">
                <span className={cn('text-base font-serif', today && 'italic')}>{formatDateShort(dateStr)}</span>
                {today && (
                  <span className="text-xs italic smallcaps text-accent">Today</span>
                )}
              </div>
              {session && (
                <Button
